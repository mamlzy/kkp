  ...
  const singlePredictionMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      toast({
        ...
        description: `Hasil: ${result.prediction === "berprestasi" ? "Berprestasi" : "Tidak Berprestasi"}`,
        variant: "success",
      });
    },
    onError: (error: Error) => {
                ...
                {modelsLoading ? (
                  ...
                  models.map((model) => (
                    <SelectItem key={model.id} value={model.id.toString()}>
                      {model.name} (Akurasi: {((model.accuracy || 0) * 100).toFixed(1)}%)
                    </SelectItem>
                  ))
                ) : (
                ...
                <div className="mt-6 rounded-lg border p-6 bg-gradient-to-br from-primary/5 to-cyan-500/5">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-sm text-muted-foreground">
                      ...
                      <span className="font-medium">
                        {formatProbability(
                          singleResult.probability[singleResult.prediction] || 0
                        )}
                      </span>
                    </div>
                  </div>
                  <div className="mt-4 grid gap-2 sm:grid-cols-2">
                    <div className="rounded-lg bg-white/50 p-3">
                      <p className="text-xs text-muted-foreground">Berprestasi</p>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground text-xs mt-1">
                      ...
                      bahasa_inggris, penjas, tik, sbk, prakarya, bahasa_sunda, btq, absen
                    </p>
                    <p className="text-muted-foreground text-xs mt-1">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid gap-4 sm:grid-cols-3">
                    <div className="rounded-lg bg-muted p-4 text-center">
                      <p className="text-2xl font-bold">{batchResult.total_count}</p>
                    ...
                    <div className="rounded-lg bg-green-50 p-4 text-center">
                      ...
                      <p className="text-sm text-muted-foreground">Berprestasi</p>
                    </div>
                    <div className="rounded-lg bg-amber-50 p-4 text-center">
                      <p className="text-2xl font-bold text-amber-600">
                        {batchResult.tidak_berprestasi_count}
                      </p>
                      <p className="text-sm text-muted-foreground">Tidak Berprestasi</p>
                    </div>
                  </div>
                      ...
                      <TableBody>
                        {batchResult.results.slice(0, 10).map((result) => (
                          <TableRow key={result.row_index}>
                            ...
                            <TableCell>
                              {formatProbability(result.probability.berprestasi || 0)}
                            </TableCell>
                            <TableCell>
                              {formatProbability(
